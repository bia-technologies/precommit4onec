...
// Возвращает признак исключения файла из обработки согласно настройкам разрешающих и исключающих регулярных выражений; 
// приоритет у списка разрешений, затем проверяется список исключений
Функция ИсключитьФайлИзОбработкиПоНастройкам(АнализируемыйФайл, ИмяФайла, 
		ИсключенныеФайлыОбработкиРегулярныеВыражения, РазрешенныеФайлыОбработкиРегулярныеВыражения) Экспорт

	Лог = МенеджерПриложения.ПолучитьЛог();
	
	Лог.Отладка("ИсключитьФайлИзОбработкиПоНастройкам(): Анализ " + ИмяФайла);

	// Имя файла, например:  src\cf\Проект\src\Documents\РасходТовара\ObjectModule.bsl
	ДанныеПоСовпадению = РегулярныеВыражения.СтрокаСовпадаетСРегулярнымиВыражениями(ИмяФайла, РазрешенныеФайлыОбработкиРегулярныеВыражения);
	Если ДанныеПоСовпадению.Совпадает Тогда
		ТекстЛога = СтрШаблон("В разрешенных файл %1 по данным настройки РазрешенныеФайлыОбработки по рег.выражению %2", 
			ИмяФайла, ДанныеПоСовпадению.РегулярноеВыражение);
		Лог.Информация(ТекстЛога);		
		Возврат Ложь;
	КонецЕсли;

	ДанныеПоСовпадению = РегулярныеВыражения.СтрокаСовпадаетСРегулярнымиВыражениями(ИмяФайла, ИсключенныеФайлыОбработкиРегулярныеВыражения);
	Если ДанныеПоСовпадению.Совпадает Тогда
		// Исключаем из файлов, т.к. файл есть в списках регулярных выражений.
		ТекстЛога = СтрШаблон("Исключаем файл %1 по данным настройки ИсключенныеФайлыОбработки по рег.выражению %2", 
			ИмяФайла, ДанныеПоСовпадению.РегулярноеВыражение);
		Лог.Информация(ТекстЛога);
		Возврат Истина;
	КонецЕсли;
