...
Функция СортироватьФайлСПростымСоставом(Знач ИмяФайла, ЭтоЕДТ, УчитываяПрефикс) Экспорт
...
Функция СортироватьМассивТеговСостава(ТекстXML, ИмяТега, РазделительЭлементов, УчитываяПрефикс, ШаблонПоискаТегов = Неопределено) Экспорт
	...
	СтрокаЗамены = СортированныеТегиСостава(СортируемаяСтрокаТегов, РазделительЭлементов, УчитываяПрефикс, ШаблонПоискаТегов);
...
Функция СортированныеТегиСостава(СортируемаяСтрокаXML, РазделительЭлементов, УчитываяПрефикс = Неопределено, Знач ШаблонПоискаТегов = Неопределено) Экспорт

	ПрефиксыСортировки  = ПрефиксыСортировки(УчитываяПрефикс);
	ТаблицыПоПрефиксам  = ТаблицыПоПрефиксам(ПрефиксыСортировки);
	
	Если Не ЗначениеЗаполнено(ШаблонПоискаТегов) Тогда // Должен содержать группу = имя объекта состава
		ШаблонПоискаТегов = "<[^>]+>([^<]*)<\/[^>]+>"; // Разбивка по тегам
	КонецЕсли;
	ПоискТегов = Создать(ШаблонПоискаТегов);
