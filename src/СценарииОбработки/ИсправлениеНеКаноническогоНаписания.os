...
Функция ИсправитьНаКаноническоеНаписание(Знач ИмяФайла)
	...
	Для Ит = 1 По ВсегоСтрок Цикл
		...
		ВРегНоваяСтрокаМодуля = " " + ВРег(НоваяСтрокаМодуля) + " ";
		ВРегНоваяСтрокаМодуля = СтрЗаменить(ВРегНоваяСтрокаМодуля, ";", " ");
		ВРегНоваяСтрокаМодуля = СтрЗаменить(ВРегНоваяСтрокаМодуля, Символы.Таб, " ");
		...
		Для Каждого ЭлементТаблицы Из КлючевыеСлова Цикл
			...
			Пока ПозицияЭталона > 0 Цикл
				...
				Написание = СокрЛП(Сред(СтрокаМодуля, СтартоваяПозиция, ДлинаЭталона));
				...
				ПозицияЭталона = СтрНайти(ВРегНоваяСтрокаМодуля, " " + Эталон + " ", , СтартоваяПозиция + ДлинаЭталона);
