...
Процедура ЗаписатьГлобальныеНастройкиПоУмолчанию(УправлениеНастройками, ТекущийКаталогСценариев)
	...
	ГлобальныеСценарии = МенеджерНастроек.ПолучитьИменаСценариевКаталога(ТекущийКаталогСценариев);
...
Процедура СброситьНастройкиРепозитория(УправлениеНастройками, ТолькоДополнительные = Ложь, ПутьКаталога = "", КаталогРепозитория = "")
	...
	Если ЗначениеЗаполнено(ПутьКаталога) Тогда
		...
		КонецЕсли;
		УправлениеНастройками.УдалитьНастройку("Precommt4onecСценарии" , ПутьКаталога);
	...
	ИначеЕсли ТолькоДополнительные Тогда 
		...
		Для Каждого Настройка Из ВсеНастройки Цикл
			...
			УправлениеНастройками.УдалитьНастройку("Precommt4onecСценарии" , Настройка);
			
		КонецЦикла;
	Иначе
		УправлениеНастройками.УдалитьНастройкиПриложения("Precommt4onecСценарии");
	КонецЕсли;
