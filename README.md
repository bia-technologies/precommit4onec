...
- `ПроверкаНецензурныхСлов` - Проверяет модули на наличие "плохих" слов, которые перечислены в файле. Пример файла `НецензурныеСлова.txt`. В v8config.json в секции `НастройкиСценариев` добавить секцию:
